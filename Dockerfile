...
FROM ghcr.io/kangwonlee/edu-scipy:0.3.3
...
RUN python3 -c "import pandas as pd; print(pd.__file__); import requests; import pytest; import glob; files = glob.glob('/tests/test_*.py'); print('Found', len(files), 'files:', files); assert files, 'No files in /tests/!'"
